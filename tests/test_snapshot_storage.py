...
from db.models import Base, TokenMarketSnapshot, TokenSecuritySnapshot
from db.repositories import store_market_snapshot, store_security_snapshot
...
def test_raw_snapshot_payloads_are_not_stored_by_default(monkeypatch) -> None:
    ...
    store_market_snapshot(
        session,
        TokenMarketData(source="dexscreener", token_address="mint", market_cap_usd=100, raw={"large": "payload"}),
    )
    store_security_snapshot(
        session,
        TokenSecurityData(source="gmgn", token_address="mint", holder_count=100, raw={"large": "payload"}),
    )
